...
<head>
    ...
    <title>Registration - Password Detail</title>
...
<body>
    ...
    <div class="container">
        <form action="/connect2local/user/admin/activities/register/code/security-detail-logic.php" method="post" class="d-flex flex-column align-items-center justify-content-center" style="height:90vh;width: 100%;">
        <div class="my-4">
        <?php

        if (isset($_SESSION['secure-key'])) {
            $key = $_SESSION['secure-key'];
            $_SESSION['key'] = $key;

            unset($_SESSION['secure-key']);
            echo "<p id='error-message' class='text-bg-dark text-center p-2 rounded m-auto message' style='line-height:30px'><i class='fa-solid fa-triangle-exclamation text-warning px-1 fs-5'></i>Your Security Key Sent to your mail</p>";
            echo "<script> 
                let message = document.querySelector('.message')
                setTimeout(function () {
                    message.style.display='none'
            }, 3000); // Close the modal after 3 seconds (3000 milliseconds) </script>";
        }
        ...
        <div class="modal-dialog modal-dialog-centered">
            ...
            <?php
                        ...
                        <script>
                            document.addEventListener('DOMContentLoaded', function () {
                                ...
                                setTimeout(function(){
                                    window.location.href='/connect2local/user/admin/activities/register/form/admin-role-approval.php';
                                },5000);
                            });
                        </script>
                        ";
            ...
            <fieldset class="p-3 border rounded" id="form-fieldset">
                <legend class="my-4 fw-bold fs-4 text-center">Account Protection</legend>
                <div class="mt-2">
                    <input type="email" name="email" class="form-control border-primary border-2" id="email" value="<?php if(isset($_SESSION['email'])) echo $_SESSION['email'];?>" readonly>
                </div>
                ...
                <div class="mt-2">
                    <input type="password" name="secure-key" id="secure-key" class="form-control" placeholder="Security Key"  required>
                </div>

                <div class="mt-3 d-flex align-items-center justify-content-between px-2">
                <div class="radio-group d-flex align-items-center justify-content-center" style="gap:7px;height:50px">
                        <input type="checkbox" name="agree-terms" id="agree-terms" class="form-input-checkbox" required>
                        <label for="agree-terms" class="form-label"><a href="/local business/webpage/policy/term-condition.php" class="nav-link">Term & Condition</a></label>
                </div>
